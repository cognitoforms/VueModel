	...
	***************************************************************************** */
	...
	function proxyEntityPropertiesOntoComponentInstance(vm, entity) {
	    ...
	    for (var i = 0; i < properties.length; i++) {
	        ...
	        if (methods && hasOwnProperty$1(methods, property.name)) {
	            debug("Property '" + property.name + "' is hidden by a component method with the same name.");
	        }
	        else if (props && hasOwnProperty$1(props, property.name)) {
	...
	var VueModel = /** @class */ (function () {
	    function VueModel(options) {
	        ...
	        var model = new Model(options.createOwnProperties);
	        // Public read-only properties
	    ...
	    VueModel.prototype.perform = function (fn) {
	        EventScope$perform(fn);
	    };
	    VueModel.prototype.onExit = function (fn) {
	...
	function VueModel$prepareOptions(options) {
	    ...
	    var result = { createOwnProperties: false, extendModel: null };
	    if (options) {
