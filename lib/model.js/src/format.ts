...
export abstract class Format<T> {
	...
	static fromTemplate<TEntity extends Entity>(type: Type, template: string): Format<TEntity> {
		return new ModelFormat<TEntity>(type, template);
	}
...
export class ModelFormat<T extends Entity> extends Format<T> {
	...
	constructor(type: Type, specifier: string) {
		super(specifier);
