...
import { expandDateFormat, formatDate, parseDate, formatNumber, parseNumber } from "./globalization";
	...
	if (type === Date) {
		...
		return Format.create<number>(model, {
			...
			convertBack: function (str: string): number | FormatError {
				...
				if (isCurrencyFormat) {
					result = parseNumber(str.replace(model.$culture.numberFormat.CurrencySymbol, ""), model.$culture) * sign;
					...
					result = parseNumber(str.replace(model.$culture.numberFormat.PercentSymbol, ""), model.$culture) / 100 * sign;
				}
				else {
					var n = parseNumber(str, model.$culture);
