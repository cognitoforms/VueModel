...
export class CustomFormat<T> extends Format<T> {
	...
	_convert: CustomFormatConvertFunction<T>;
	_convertBack: CustomFormatConvertBackFunction<T>;
	_formatEval: (value: string) => string;
	_paths: string[];
	...
	constructor(options: CustomFormatOptions<T>) {
		...
		Object.defineProperty(this, "_convert", { enumerable: false, value: options.convert, writable: true });
		Object.defineProperty(this, "_convertBack", { enumerable: false, value: options.convertBack, writable: true });
		Object.defineProperty(this, "_formatEval", { enumerable: false, value: options.formatEval, writable: true });
		Object.defineProperty(this, "_paths", { enumerable: false, value: options.paths, writable: true });
	}
...
export class ModelFormat<T extends Entity> extends Format<T> {
	...
	constructor(type: Type, specifier: string, formatEval: unknown = null) {
		...
		Object.defineProperty(this, "_compiled", { configurable: false, enumerable: false, value: false, writable: true });
		Object.defineProperty(this, "_tokens", { configurable: false, enumerable: false, value: null, writable: true });
		Object.defineProperty(this, "_template", { configurable: false, enumerable: false, value: null, writable: true });
		Object.defineProperty(this, "_paths", { configurable: false, enumerable: false, value: null, writable: true });

	}

	_ensureCompiled(): void {
		if (!this._compiled) {
			if (!this._tokens) {
				this._paths = [];
				this._tokens = [];
				...
				let template = this._template.replace(/\\\\/g, '\u0000').replace(/\\\[/g, '\u0001').replace(/\\\]/g, '\u0002');
				var index = 0;
			...
			if (token.path) {
				...
				if (value === undefined || value === null) {
					...
					if (token.format instanceof Format) {
						...
						format = token.format = getFormat<any>(obj.meta.type.model, value.constructor, token.format);
					}
...
export interface FormatLookupForType {
	type: any;
	formats: { [name: string]: Format<any> };
}
...
function getFormatStorage(model: Model, type: EntityConstructorForType<Entity> | any): { [name: string]: Format<any> }  {

	if (type.meta && type.meta instanceof Type) {
		// Entity types track their own formats
		let entityCtor = type as EntityConstructorForType<Entity>;
		return entityCtor.meta._formats;
	}

	let nativeTypeName: string = null;
	if (type === String) {
	...
	if (nativeTypeName) {
		...
		let formatStorageForNativeType = model._nativeTypeFormats[nativeTypeName];
		...
		let unknownTypeFormats: FormatLookupForType[];
		if (hasOwnProperty(model, "_unknownTypeFormats")) {
			unknownTypeFormats = (model as any)._unknownTypeFormats;
		...
		let formatStorage = {};
		unknownTypeFormats.push({ type, formats: formatStorage });
...
export function getFormat<T>(model: Model, type: EntityConstructorForType<Entity> | Function, format: string): Format<T> {
	...
	let formats = getFormatStorage(model, type);
